...
pub struct Database {
    inner: Arc<DB>,
    views_writer: Mutex<WriteHandle<String, Box<DatabaseView<Arc<DB>>>>>,
    views_reader: ReadHandle<String, Box<DatabaseView<Arc<DB>>>>,
}
...
impl Database {
    pub fn create<P: AsRef<Path>>(path: P) -> Result<Database, Box<Error>> {
        ...
        let (views_reader, views_writer) = evmap::new();

        let database = Database {
            inner: Arc::new(db),
            views_writer: Mutex::new(views_writer),
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Database, Box<Error>> {
        ...
        let (views_reader, mut views_writer) = evmap::new();
        let cf_map = db.cf_map();

        for name in cfs {
            if name == "default" { continue }

            let snapshot = Snapshot::new(db.clone());
            let view = DatabaseView::new(snapshot, cf_map.clone(), name.clone())?;
            views_writer.update(name.clone(), Box::new(view));
        }

        views_writer.refresh();
    ...
    pub fn create_index(&self, name: String, schema: &Schema) -> Result<(), Box<Error>> {
        let mut cf_opts = ColumnFamilyOptions::new();
        cf_opts.add_merge_operator("data-index merge operator", merge_indexes);

        let cf_descriptor = ColumnFamilyDescriptor::new(&name, cf_opts);
        self.inner.create_cf(cf_descriptor)?;

        let mut schema_bytes = Vec::new();
        schema.write_to_bin(&mut schema_bytes)?;

        let cf_map = self.inner.cf_map();
        let result = cf_map.get_and(&name, |values| {
            assert_eq!(values.len(), 1);

            let handle = &values[0].handle;

            let writeopts = WriteOptions::new();
            self.inner.put_cf_opt(handle, DATA_SCHEMA, &schema_bytes, &writeopts)?;
            let snapshot = Snapshot::new(self.inner.clone());
            let view = DatabaseView::new(snapshot, cf_map.clone(), name.clone())?;

            let mut lock = self.views_writer.lock().unwrap();
            lock.update(name.clone(), Box::new(view));
            lock.refresh();
    ...
    pub fn index_map(&self) -> &ReadHandle<String, Box<DatabaseView<Arc<DB>>>> {
        &self.views_reader
    }

    pub fn update_index(&self, name: String, update: Update) -> Result<(), Box<Error>> {
        let cf_map = self.inner.cf_map();
        let result = cf_map.get_and(&name, |values| {
            assert_eq!(values.len(), 1);
            ...
            let handle = &values[0].handle;
            self.inner.ingest_external_file_optimized(handle, &options, &[&path])?;

            // Compacting to trigger the merge operator only one time
            // while ingesting the update and not each time searching
            self.inner.compact_range(Some(DATA_INDEX), Some(DATA_INDEX));
