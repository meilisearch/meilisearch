...
use index_scheduler::filter::{
    filter_into_index_filter, filters_into_index_filters, retrieve_foreign_keys_settings,
    SourceIndexUid,
};
...
use meilisearch_types::milli::{
    filtered_universe, AttributeState, Deadline, FacetValueHit, IndexFilter, InternalError,
...
pub fn perform_similar(
    ...
    let docid_filter = docid_filter
        .as_ref()
        .map(|docid_filter| parse_filter(docid_filter, Code::InvalidSimilarFilter, features))
        .transpose()?
        .flatten();

    let candidates_filter = filter
        .as_ref()
        .map(|filter| parse_filter(filter, Code::InvalidSimilarFilter, features))
        .transpose()?
        .flatten();
