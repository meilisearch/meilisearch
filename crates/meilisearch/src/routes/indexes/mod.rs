...
#[routes::routes(
    routes(
        ...
        "/{indexUid}" => [get(get_index), patch(update_index), delete(delete_index)],
        "/{indexUid}/documents" => sub(documents::DocumentsApi),
        "/{indexUid}/facet-search" => sub(facet_search::FacetSearchApi),
        "/{indexUid}/similar" => sub(similar::SimilarApi),
        "/{indexUid}/settings" => sub(settings::SettingsApi),
        "/{indexUid}/compact" => sub(compact::CompactApi),
        "/{indexUid}/search" => sub(search::SearchApi),
        "/{indexUid}/stats" => get(get_index_stats),
        "/{indexUid}/fields" => post(fields::post_index_fields),
    ),
    tag = "Indexes",
    tags(
        (
