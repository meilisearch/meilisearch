...
async fn list_rules(
    ...
    let rules = index_scheduler.search_dynamic_rules();
...
async fn create_rule(
    ...
    if index_scheduler.get_search_dynamic_rule(&body.uid)?.is_some() {
...
async fn update_rule(
    ...
    let mut rule = index_scheduler
        .get_search_dynamic_rule(&uid)?
        .ok_or_else(|| DynamicSearchRulesError::NotFound(uid.clone()))?;
...
async fn delete_rule(
    ...
    let uid = uid.into_inner();
    let deleted = index_scheduler.delete_search_dynamic_rule(&uid)?;
