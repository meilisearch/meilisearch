...
fn check_rule(rule: &DynamicSearchRuleUpdateRequest) -> Result<(), ResponseError> {
    if let Setting::Set(actions) = &rule.actions {
        for (action_index, scale) in actions.scale.iter().enumerate() {
            let empty_filter = 'check_filter: {
                let Some(filter) = &scale.filter else {
                    break 'check_filter true;
                };
                let Some(filter) = Filter::from_json(filter).map_err(|err| {
                ...
                break 'check_filter false;
            };

            if empty_filter && scale.ids.as_ref().is_none_or(|ids| ids.is_empty()) {
                return Err(ResponseError::from_msg(format!("invalid `.actions.scale[{action_index}]`: one of `filter` or `ids` must be a non-empty valid value."), Code::InvalidDynamicSearchRuleActions));
            }
        }
    }
