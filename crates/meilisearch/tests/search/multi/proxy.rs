...
async fn error_feature() {
    ...
    {
      "message": "Performing a proxy search requires enabling the `proxy search` experimental feature. See https://github.com/orgs/meilisearch/discussions/805",
    ...
    snapshot!(json_string!(response), @r###"
    {
      "message": "Using `federationOptions.queryPosition` requires enabling the `proxy search` experimental feature. See https://github.com/orgs/meilisearch/discussions/805",
      "code": "feature_not_enabled",
      "type": "invalid_request",
      "link": "https://docs.meilisearch.com/errors#feature_not_enabled"
    }
...
async fn remote_sharding() {
    ...
    let (response, code) = ms0.set_features(json!({"proxySearch": true})).await;
    snapshot!(code, @"200 OK");
    snapshot!(json_string!(response["proxySearch"]), @"true");
    let (response, code) = ms1.set_features(json!({"proxySearch": true})).await;
    snapshot!(code, @"200 OK");
    snapshot!(json_string!(response["proxySearch"]), @"true");
    let (response, code) = ms2.set_features(json!({"proxySearch": true})).await;
    snapshot!(code, @"200 OK");
    snapshot!(json_string!(response["proxySearch"]), @"true");
...
async fn error_no_weighted_score() {
    ...
    let (response, _status_code) = ms0.multi_search(request.clone()).await;
    snapshot!(code, @"200 OK");
    // This is **not** returning anything in `remoteErrors` yet
    // because the implementation only WARN on that case
    // Instead, hits are simply ignored
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      "hits": [
        {
    ...
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      ...
      "remoteErrors": {
        "ms1": {
          ...
          "type": "system",
          "link": "https://docs.meilisearch.com/errors#proxy_bad_response"
        }
      }
    }
    "###);
...
// test: remote error bad request (for instance, indexUid is unknown on the remote instance)
#[actix_rt::test]
    ...
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      ...
      "remoteErrors": {
        "ms1": {
          ...
          "type": "invalid_request",
          "link": "https://docs.meilisearch.com/errors#proxy_bad_request"
        }
      }
    }
    "###);
...
// test: remote error bad request (for instance, facetsByIndex has an unknown index)
#[actix_rt::test]
    ...
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      ...
      "remoteErrors": {
        "ms2": {
          ...
          "type": "system",
          "link": "https://docs.meilisearch.com/errors#proxy_could_not_send_request"
        }
      }
    }
    "###);
    ...
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      ...
      "remoteErrors": {
        "ms1-notsearch": {
          ...
          "code": "proxy_invalid_api_key",
          "type": "auth",
          "link": "https://docs.meilisearch.com/errors#proxy_invalid_api_key"
        },
        "ms1-nottest": {
          "message": "could not authenticate against the remote host",
          "code": "proxy_invalid_api_key",
    ...
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      ...
      "remoteErrors": {
        "ms1": {
          ...
          "code": "proxy_remote_error",
          "type": "system",
          "link": "https://docs.meilisearch.com/errors#proxy_remote_error"
        }
      }
    }
    "###);
    ...
    snapshot!(json_string!(response, { ".processingTimeMs" => "[time]" }), @r###"
    {
      ...
      "remoteErrors": {
        "ms1": {
          ...
          "code": "proxy_timeout",
          "type": "system",
          "link": "https://docs.meilisearch.com/errors#proxy_timeout"
        }
      }
    }
    "###);
...
/// A server that exploits [`MockServer`] to provide an URL for testing proxy search and the network.
