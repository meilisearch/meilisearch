...
#[actix_rt::test]
async fn error_proxy_search_not_enabled() {
    ...
    {
      "message": "Using the /network route requires enabling the `proxy search` experimental feature. See https://github.com/orgs/meilisearch/discussions/805",
...
async fn errors_on_param() {
    ...
    let (response, code) = server.set_features(json!({"proxySearch": true})).await;
    meili_snap::snapshot!(code, @"200 OK");
    meili_snap::snapshot!(meili_snap::json_string!(response["proxySearch"]), @r#"true"#);
