...
pub struct EmbeddingSettings {
    ...
    /// Embedder used at search time (for composite embedders).
    #[serde(default, skip_serializing_if = "Setting::is_not_set")]
    #[deserr(default)]
    #[schema(value_type = Option<SubEmbeddingSettings>)]
    pub search_embedder: Setting<SubEmbeddingSettings>,

    /// Embedder used at indexing time (for composite embedders).
    #[serde(default, skip_serializing_if = "Setting::is_not_set")]
    #[deserr(default)]
    #[schema(value_type = Option<SubEmbeddingSettings>)]
    pub indexing_embedder: Setting<SubEmbeddingSettings>,

    /// Affine transform (mean, sigma) for semantic score so it aligns with ranking score. Example: `{"mean": 0.7, "sigma": 0.3}`.
    #[serde(default, skip_serializing_if = "Setting::is_not_set")]
    #[deserr(default)]
