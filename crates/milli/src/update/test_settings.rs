...
fn set_asc_desc_field() {
    ...
    let rtxn = index.read_txn().unwrap();
    let SearchResult { documents_ids, .. } = index.search(&rtxn, |_| {}).execute().unwrap();
