...
fn must_update_database<SD: SettingsDelta>(settings_delta: &SD) -> bool {
    // if any embedder action has a write_back, we need to update the documents database
    settings_delta.embedder_actions().values().any(|action| action.write_back().is_some())
}
    ...
    's: 'a,
{
    ...
    for (name, action) in embedder_actions {
        ...
        // if the vector entry is user provided, we need to update the document
        if !vector_entry.regenerate {
            return Ok(true);
        }
    }
