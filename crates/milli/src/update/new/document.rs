...
pub trait Document<'doc> {
    ...
    where
        ...
        E: FnMut(serde_json::Error) -> crate::Error,
        Self: Sized,
    {
        facet_values(self, permissive_json_pointer, bump, &mut |raw_value| {
            for facet_value in RawFacetValue::from_value(raw_value, bump) {
                let facet_value = match facet_value {
                    Ok(facet_value) => facet_value,
                    Err(err) => return Err(on_json_error(err)),
                };
                visit(facet_value)?;
            }
            Ok(())
...
impl<'doc> RawFacetValue<'doc> {
    fn from_value(
        ...
        bump: &'doc Bump,
    ) -> impl Iterator<Item = Result<Self, serde_json::Error>> + 'doc {
        match bumparaw_collections::Value::from_raw_value(raw_value, bump) {
            Ok(bumparaw_collections::Value::Array(values)) => {
                either::Either::Left(values.into_iter().flat_map(|raw_value| {
                    match bumparaw_collections::Value::from_raw_value(raw_value, bump) {
                        Ok(value) => Self::from_leaf_value(value).map(Ok),
                        Err(err) => Some(Err(err)),
                    }
                }))
            }
            Ok(value) => either::Either::Right(Self::from_leaf_value(value).map(Ok).into_iter()),
            Err(err) => either::Either::Right(Some(Err(err)).into_iter()),
        }
    }
