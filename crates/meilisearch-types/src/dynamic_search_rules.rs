...
pub fn parse_index_filter(
    ...
    let filter = milli::Filter::from_str(&expression)
        .map_err(|e| ParseIndexFilterError(e.to_string()))?
        .ok_or_else(|| ParseIndexFilterError("filter cannot be empty".to_string()))?;

    milli::condition_to_index_condition(filter.condition, &mut |_| {
        Err(milli::Error::UserError(milli::UserError::InvalidFilter(
            "foreign filters are not supported in dynamic search rule filter conditions"
                .to_string(),
        )))
    })
    .map(|condition| IndexFilter { condition }.into_owned())
    .map_err(|e| ParseIndexFilterError(e.to_string()))
}
