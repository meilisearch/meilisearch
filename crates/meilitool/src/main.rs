...
use meilisearch_types::milli::{obkv_to_json, CreateOrOpen, DeCboRoaringBitmapCodec, BEU32};
...
fn index_cbo_databases(
    ...
    for result in index_mapping.iter(&index_mapper_rtxn)? {
        ...
        let index = Index::new(EnvOpenOptions::new().read_txn_without_tls(), &index_path, CreateOrOpen::Open)
            .with_context(|| {
