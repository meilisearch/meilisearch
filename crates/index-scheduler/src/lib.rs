...
use meilisearch_types::heed::{self, Env, RoTxn, RwTxn};
    ...
    pub fn get_stats(&self) -> Result<BTreeMap<String, BTreeMap<String, u64>>> {
        ...
        res.insert(
            ...
            enum_iterator::all::<Status>()
                .map(|s| {
                    let tasks = self.queue.tasks.get_status(&rtxn, s)?.len();
                    match s {
        ...
        res.insert(
            ...
            enum_iterator::all::<Kind>()
                .map(|s| Ok((s.to_string(), self.queue.tasks.get_kind(&rtxn, s)?.len())))
                .collect::<Result<BTreeMap<String, u64>>>()?,
        );
        res.insert(
            "indexes".to_string(),
    ...
    fn tick(&self) -> Result<TickOutcome> {
        #[cfg(test)]
        {
            *self.run_loop_iteration.write().unwrap() += 1;
            self.breakpoint(test_utils::Breakpoint::Start);
        }
        ...
        #[cfg(test)]
        self.breakpoint(test_utils::Breakpoint::BatchCreated);
        ...
        #[cfg(test)]
        self.maybe_fail(test_utils::FailureLocation::AcquiringWtxn)?;
        ...
        match res {
            Ok(tasks) => {
                #[cfg(test)]
                self.breakpoint(test_utils::Breakpoint::ProcessBatchSucceeded);
                ...
                for (i, mut task) in tasks.into_iter().enumerate() {
                    ...
                    #[cfg(test)]
                    self.maybe_fail(
                        test_utils::FailureLocation::UpdatingTaskAfterProcessBatchSuccess {
                            task_uid: i as u32,
                        },
                    )?;
            ...
            | Err(Error::AbortedTask) => {
                #[cfg(test)]
                self.breakpoint(test_utils::Breakpoint::AbortedIndexation);
                wtxn.abort();
            ...
            Err(err) => {
                #[cfg(test)]
                self.breakpoint(test_utils::Breakpoint::ProcessBatchFailed);
                ...
                for id in ids.iter() {
                    ...
                    #[cfg(test)]
                    self.maybe_fail(
                        test_utils::FailureLocation::UpdatingTaskAfterProcessBatchFailure,
                    )?;
        ...
        #[cfg(test)]
        self.maybe_fail(test_utils::FailureLocation::CommittingWtxn)?;
        ...
        #[cfg(test)]
        self.breakpoint(test_utils::Breakpoint::AfterProcessing);
