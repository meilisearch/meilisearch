...
struct Cli {
    ...
    /// Check that non-optional params have explicit required, and default+required is consistent (useful for CI)
    #[arg(long)]
    check_required_params: bool,
}
...
fn main() -> Result<()> {
    ...
    // Check that parameters/properties with default are not required
    if cli.check_required_params {
        check_required_params(&openapi_value)?;
    }
...
/// Returns the schema type string ("string", "integer", "array", "object", etc.)
/// after resolving $ref. Returns None if no type or unresolved.
fn schema_type<'a>(openapi: &'a Value, schema: &'a Value) -> Option<&'a str> {
    let resolved = if let Some(r#ref) = schema.get("$ref").and_then(|r| r.as_str()) {
        resolve_ref(openapi, r#ref).unwrap_or(schema)
    } else {
        schema
    };
...
/// Returns true if the schema represents an optional type (Option<T> in Rust),
/// i.e. nullable or oneOf/anyOf that includes null.
fn schema_is_optional_type(openapi: &Value, schema: &Value) -> bool {
    let resolved = if let Some(r#ref) = schema.get("$ref").and_then(|r| r.as_str()) {
        resolve_ref(openapi, r#ref).unwrap_or(schema)
    } else {
        schema
    };
    if resolved.get("nullable").and_then(|n| n.as_bool()).unwrap_or(false) {
        return true;
    }
    for key in ["oneOf", "anyOf"] {
        if let Some(arr) = resolved.get(key).and_then(|a| a.as_array()) {
            for branch in arr {
                if let Some(obj) = branch.as_object() {
...
/// Checks that parameters and request body schemas with non-optional types
/// have "required" explicitly set (so the OpenAPI spec is unambiguous).
///
/// - For query/header parameters: if the parameter's schema type is not optional
///   (not nullable), the parameter object must have a "required" field.
/// - For request body: if the schema has "required" array, properties with default must not be in it.
/// - Also: parameters/properties with a default value must not be required.
fn check_required_params(openapi: &Value) -> Result<()> {
    let paths = openapi
        .get("paths")
        .and_then(|p| p.as_object())
        .context("OpenAPI spec missing 'paths' object")?;
        ...
        for method in HTTP_METHODS {
            ...
            for param in params {
                ...
                let param_in = param_obj
                    ...
                    .and_then(|i| i.as_str())
                    .unwrap_or("unknown");
                // Path params are always required; only enforce explicit "required" for query/header
                if param_in != "query" && param_in != "header" {
                    continue;
                }
                let schema = param_obj.get("schema").unwrap_or(&serde_json::Value::Null);
                if schema_is_optional_type(openapi, schema) {
                    continue;
                }
                // Non-optional type => "required" must be explicit
                if !param_obj.contains_key("required") {
                    errors.push(format!(
                        "{}: parameter \"{}\" (in: {}) has a non-optional type but \"required\" is not explicit. \
                         Add required = true or required = false in #[param(...)].",
                        prefix, name, param_in
                    ));
                    continue;
                }
                // If it has a default, it must not be required
                let has_default = param_obj.get("default").is_some()
                    || schema.get("default").is_some();
                let required = param_obj.get("required").and_then(|r| r.as_bool()).unwrap_or(false);
                if has_default && required {
                    errors.push(format!(
                        "{}: parameter \"{}\" (in: {}) has a default value but is generated as required (required: true). \
                         Add required = false in #[param(...)].",
                        prefix, name, param_in
                    ));
                }
                // array/object/boolean query params are typically optional with utoipa (concrete type => required: true by default).
                if required {
                    let ty = schema_type(openapi, schema);
                    if ty == Some("array") || ty == Some("object") {
                        errors.push(format!(
                            "{}: parameter \"{}\" (in: {}) has type {:?} but is generated as required (required: true). \
                             If the param is optional (e.g. Option<T> in Rust), add required = false in #[param(...)].",
                            prefix, name, param_in, ty
                        ));
                    } else if ty == Some("boolean") {
                        errors.push(format!(
                            "{}: parameter \"{}\" (in: {}) is boolean and generated as required (required: true). \
                             Booleans are required by default with utoipa; add required = false in #[param(...)] if optional.",
                            prefix, name, param_in
                        ));
                    }
                }
            ...
            // Request body: schema with properties must have "required" array
            if let Some(req_body) = operation.get("requestBody") {
                let content = match req_body.get("content").and_then(|c| c.get("application/json")) {
                    Some(c) => c,
                    None => continue,
                };
                let schema = match content.get("schema") {
                    Some(s) => s,
                    None => continue,
                };
                let resolved_schema = if let Some(r#ref) = schema.get("$ref").and_then(|r| r.as_str()) {
                    resolve_ref(openapi, r#ref).unwrap_or(schema)
                } else {
                    schema
                };
                if let Some(properties) = get_schema_properties(openapi, resolved_schema) {
                    if let Some(required_names) = resolved_schema.get("required").and_then(|r| r.as_array()) {
                        let required_set: std::collections::HashSet<_> =
                            required_names.iter().filter_map(|v| v.as_str()).collect();
                        for (prop_name, prop_value) in properties {
                            let in_required = required_set.contains(prop_name.as_str());
                            if prop_value.get("default").is_some() && in_required {
                                errors.push(format!(
                                    "{}: request body property \"{}\" has a default value but is in the required list. \
                                     Add #[schema(required = false)] to the field.",
                                    prefix, prop_name
                                ));
                            } else if in_required {
                                let prop_type = prop_value.get("type").and_then(|t| t.as_str());
                                if prop_type == Some("boolean") {
                                    errors.push(format!(
                                        "{}: request body property \"{}\" is boolean and in the required list. \
                                         Booleans are required by default with utoipa; add #[schema(required = false)] if optional.",
                                        prefix, prop_name
                                    ));
                                }
                            }
    ...
    if errors.is_empty() {
        println!("All non-optional parameters have explicit \"required\", and default+required is consistent.");
        Ok(())
    } else {
        errors.sort();
        eprintln!("Non-optional types must have explicit \"required\"; parameters with default must not be required:\n");
        for e in &errors {
            eprintln!("  - {}", e);
        }
        eprintln!("\nFix: add required = true or required = false in #[param(...)]; add #[schema(required = false)] where needed.");
        anyhow::bail!("{} required/default inconsistency(ies)", errors.len());
    }
}
...
mod tests {
    ...
    #[test]
    fn test_check_required_params_parameter_with_default_and_required_fails() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "get": {
        ...
        let result = check_required_params(&openapi);
        assert!(result.is_err(), "parameter with default and required=true must be rejected");
        let err = result.unwrap_err().to_string();
        assert!(err.contains("required"), "error should mention required");
    }

    #[test]
    fn test_check_required_explicit_non_option_query_param_missing_required_fails() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "get": {
        ...
        let result = check_required_params(&openapi);
        assert!(result.is_err(), "query param with non-optional type must have explicit required");
        let err = result.unwrap_err().to_string();
        assert!(err.contains("required"), "error should mention required");
    }

    #[test]
    fn test_check_required_explicit_non_option_query_param_with_required_passes() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "get": {
    ...
    #[test]
    fn test_check_array_param_required_true_fails() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "get": {
        ...
        let result = check_required_params(&openapi);
        assert!(result.is_err(), "query param with type array and required: true must be rejected");
        let err = result.unwrap_err().to_string();
        assert!(err.contains("required"), "error should mention required");
    }

    #[test]
    fn test_check_boolean_param_required_true_fails() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "get": {
        ...
        let result = check_required_params(&openapi);
        assert!(result.is_err(), "query param boolean with required: true must be rejected");
        let err = result.unwrap_err().to_string();
        assert!(err.contains("required"), "error should mention required");
    }

    #[test]
    fn test_check_boolean_body_property_required_fails() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "post": {
        ...
        let result = check_required_params(&openapi);
        assert!(result.is_err(), "body property boolean in required list must be rejected");
        let err = result.unwrap_err().to_string();
        assert!(err.contains("required"), "error should mention required");
    }

    #[test]
    fn test_check_required_params_parameter_with_default_and_not_required_passes() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "get": {
    ...
    #[test]
    fn test_check_required_params_request_body_property_with_default_and_required_fails() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "post": {
        ...
        let result = check_required_params(&openapi);
        assert!(result.is_err(), "request body property with default in required list must be rejected");
        let err = result.unwrap_err().to_string();
        assert!(err.contains("required"), "error should mention required");
    }

    #[test]
    fn test_check_required_params_request_body_property_with_default_not_in_required_passes() {
        let openapi = json!({
            "paths": {
                "/search": {
                    "post": {
