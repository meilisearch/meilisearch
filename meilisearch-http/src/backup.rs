...
impl BackupMetadata {
    ...
    pub fn new(indexes: Vec<IndexResponse>, db_version: String) -> Self {
        BackupMetadata {
            indexes,
            db_version,
...
pub fn import_backup(
    ...
    //TODO: Make transactionnal
    let existing_index_uids = data.db.load().indexes_uids();
    for index in metadata.indexes.iter() {
        if existing_index_uids.contains(&index.uid) {
            data.db.load().delete_index(index.uid.clone())?;
        }
        let create_request = IndexCreateRequest {
            uid: Some(index.uid.clone()),
            name: Some(index.name.clone()),
            primary_key: index.primary_key.clone()
        };
        data.create_index(&create_request).map_err(|e| Error::create_index(e))?;
    }

    // import each indexes content
    data.db.load().main_write::<_, _, Error>(|mut writer| {
...
fn backup_metadata(data: &web::Data<Data>, folder_path: &Path, indexes: Vec<IndexResponse>) -> Result<(), Error> {
    let (db_major, db_minor, db_patch) = data.db.load().version();
...
fn backup_index_settings(data: &web::Data<Data>, reader: &MainReader, folder_path: &Path, index_uid: &str) -> Result<(), Error> {
    let settings = data.get_all_settings_sync(index_uid, reader)?;
...
fn backup_index_updates(data: &web::Data<Data>, reader: &UpdateReader, folder_path: &Path, index_uid: &str) -> Result<(), Error> {
    ...
    let updates = data.get_all_updates_status_sync(reader, index_uid)?;
...
fn backup_index_documents(data: &web::Data<Data>, reader: &MainReader, folder_path: &Path, index_uid: &str) -> Result<(), Error> {
    ...
    let mut offset = 0;
    loop {
        let documents = data.get_all_documents_sync(reader, index_uid, offset, backup_batch_size, None)?;
...
fn backup_process(data: web::Data<Data>, backup_folder: PathBuf, backup_info: BackupInfo) {
    ...
    let update_reader = match data.db.load().update_read_txn() {
        Ok(r) => r,
        Err(e) => {
    ...
    let main_reader = match data.db.load().main_read_txn() {
        Ok(r) => r,
        Err(e) => {
    ...
    // fetch indexes
    let indexes = match data.list_indexes_sync(&main_reader) {
        Ok(indexes) => indexes,
        Err(e) => {
