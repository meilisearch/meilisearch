...
fn parse_filter<'a>(facets: &'a Value) -> Result<Option<Filter<'a>>> {
    match facets {
        Value::String(expr) => {
...
fn parse_filter_array<'a>(arr: &'a [Value]) -> Result<Option<Filter<'a>>> {
    let mut ands = Vec::new();
    for value in arr {
        match value {
            Value::String(s) => ands.push(Either::Right(s.as_ref())),
            Value::Array(arr) => {
                let mut ors = Vec::new();
                for value in arr {
                    match value {
                        Value::String(s) => ors.push(s.as_ref()),
                        v => {
